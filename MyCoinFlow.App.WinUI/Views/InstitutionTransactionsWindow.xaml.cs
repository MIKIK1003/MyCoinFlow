...
public sealed partial class InstitutionTransactionsWindow : PersistentWindow
{
    ...
    private readonly GeldinstitutTransaktionenViewModel _viewModel;

    public InstitutionTransactionsWindow(Geldinstitut institution)
    {
        InitializeComponent();
        ...
        _viewModel = new GeldinstitutTransaktionenViewModel(institution.Id, institution.Name);
    ...
    {
        var rows = _viewModel.Rows.Select(row => new InstitutionTransactionDisplayRow(row)).ToList();
    ...
    private void OnSelectionChanged(object sender, SelectionChangedEventArgs e) =>
        EditButton.IsEnabled = TransactionsList.SelectedItem is InstitutionTransactionDisplayRow;

    private async void OnEditClick(object sender, RoutedEventArgs e)
    {
        if (TransactionsList.SelectedItem is not InstitutionTransactionDisplayRow selected) return;
        try
        {
    ...
    private void OnPrintClick(object sender, RoutedEventArgs e)
    {
        try
        {
            ...
            dialog.PrintDocument(((IDocumentPaginatorSource)document).DocumentPaginator, "Geldinstitut-Transaktionen");
        }
        catch (Exception exception)
        {
    ...
    private FlowDocument BuildPrintDocument(double printableWidth, double printableHeight)
    {
        ...
        document.Blocks.Add(new Paragraph(new Bold(new Run("Transaktionen – Geldinstitut")))
        {
        ...
        foreach (var width in new[] { 190d, 50d, 100d, 100d, 240d, 380d })
        ...
        foreach (var text in new[] { "Konto", "Datum", "Einnahmen", "Ausgaben", "Adresse", "Notiz" })
    ...
    private string BuildFilterLine()
    {
        ...
        return $"{_viewModel.Titel}   |   Zeitraum: {period}   |   " +
               $"Einnahmen {_viewModel.SummeEinnahmen.ToString("N2", SwissCulture)}   |   " +
               $"Ausgaben {_viewModel.SummeAusgaben.ToString("N2", SwissCulture)}   |   " +
