...
{
    public class KontenGruppeViewModel : INotifyPropertyChanged
    {
        public ObservableCollection<KontenGruppe> KontenGruppen { get; set; } = new();

        private KontenGruppe? _ausgewaehlteGruppe;
        public KontenGruppe? AusgewaehlteGruppe
        {
        ...
        public KontenGruppeViewModel()
        {
            ...
            LoeschenCommand = new RelayCommand(
                _ => Loeschen(),
                _ => AusgewaehlteGruppe != null);

            BearbeitenCommand = new RelayCommand(
                _ => Bearbeiten(),
                _ => AusgewaehlteGruppe != null
                     && !string.IsNullOrWhiteSpace(BearbeiteteBezeichnung)
                     && !string.Equals(AusgewaehlteGruppe?.Bezeichnung, BearbeiteteBezeichnung.Trim(), StringComparison.CurrentCulture));
        ...
        {
            if (AusgewaehlteGruppe is null) return;

            var ask = MessageBox.Show(
                $"Soll die Gruppe „{AusgewaehlteGruppe.Bezeichnung}“ wirklich gelöscht werden?\n" +
            ...
            var db = new DatabaseService();
            db.LoescheKontenGruppe(AusgewaehlteGruppe.Id);
        ...
        private void Bearbeiten()
        {
            ...
            var oldName = AusgewaehlteGruppe.Bezeichnung;
            ...
            var ask = MessageBox.Show(
                ...
                "• die Stammdaten-Tabelle KontenGruppe (Bezeichnung)\n" +
                "• ALLE betroffenen Zeilen im Kontenplan (Feld Gruppe)\n" +
                "aus. Fortfahren?",
            ...
            try
            {
                ...
                db.RenameKontenGruppe(oldName, newName);

                // 2) UI aktualisieren
                MessageBox.Show("Gruppe wurde erfolgreich umbenannt und im Kontenplan aktualisiert.", "Erfolg",
